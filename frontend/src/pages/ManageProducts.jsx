...
const ManageProducts = () => {
  ...
  const handleConfirmUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/products/${selectedProduct.product_id}`, {
        method: 'PUT',
        body: dataToSend
      });
                      ...
                      <td className="p-4">
                        <div className="w-12 h-12 rounded-md border border-slate-200 overflow-hidden bg-white flex items-center justify-center">
                          ...
                          {product.image_preview_temp ? (
                            ...
                            <img src={`http://localhost:5000/uploads/${product.image}`} alt={product.product_name} className="w-full h-full object-cover" />
                          ) : (
