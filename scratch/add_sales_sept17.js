...
async function addSalesSept17() {
    try {
        console.log('Starting to add sales for Sept 17 (06:00 - 15:00)...');
        ...
        const numSales = Math.floor(Math.random() * 16) + 25; // 25-40 sales
        console.log(`Generating ${numSales} sales...`);
        
        // Generate timestamps for Sept 17, 06:00 - 15:00 (TH Time)
        // UTC: 2026-09-16 23:00:00 to 2026-09-17 08:00:00
        const timestamps = [];
        const minTime = new Date('2026-09-16T23:00:00Z').getTime(); 
        const maxTime = new Date('2026-09-17T08:00:00Z').getTime();
        ...
        // Fetch current inventory and filter out products restocked on Sept 17 (TH Time)
        const [inventoryRows] = await db.query(`
            ...
            AND p.product_id NOT IN (
                SELECT product_id 
                FROM stock_in 
                WHERE DATE(DATE_ADD(created_at, INTERVAL 7 HOUR)) >= '2026-09-17'
            )
        `);
        ...
        for (const ts of timestamps) {
            ...
            for (let j = 0; j < numItems; j++) {
                ...
                // Also bounded by 6 (global max) and remaining inventory
        ...
        console.log(`Successfully generated ${numSales} sales for Sept 17 with strict inventory and logical checks.`);
        process.exit(0);
