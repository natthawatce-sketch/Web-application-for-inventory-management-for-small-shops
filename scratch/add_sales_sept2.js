...
async function addSalesSept2() {
    try {
        console.log('Starting to add sales for Sept 2...');
        ...
        function getWeightsForHour(hour) {
            if (hour >= 6 && hour < 10) {
                // Morning (6-10): Coffee, breakfast
                return [
                    { catId: 1, weight: 50 }, // Drinks
                    { catId: 2, weight: 40 }, // Food
                    { catId: 4, weight: 5 },  // Utilities
                    { catId: 3, weight: 2 },  // Condiments
                    { catId: 5, weight: 3 }   // Medicine
                ];
            } else if (hour >= 10 && hour < 16) {
                // Daytime (10-16): Snacks, drinks
                return [
                    { catId: 1, weight: 40 }, // Drinks
                    { catId: 2, weight: 40 }, // Food
                    { catId: 4, weight: 10 }, // Utilities
                    { catId: 3, weight: 8 },  // Condiments
                    { catId: 5, weight: 2 }   // Medicine
                ];
            } else if (hour >= 16 && hour < 20) {
                // Evening (16-20): Cooking dinner, utilities
                return [
                    { catId: 3, weight: 40 }, // Condiments
                    { catId: 2, weight: 25 }, // Food
                    { catId: 1, weight: 20 }, // Drinks
                    { catId: 4, weight: 12 }, // Utilities
                    { catId: 5, weight: 3 }   // Medicine
                ];
            } else {
                // Night (20-23): Late night snacks, medicines
                return [
                    { catId: 2, weight: 45 }, // Food
                    { catId: 1, weight: 35 }, // Drinks
                    { catId: 4, weight: 10 }, // Utilities
                    { catId: 5, weight: 8 },  // Medicine
                    { catId: 3, weight: 2 }   // Condiments
                ];
            }
        }
        ...
        for (const ts of timestamps) {
            // Determine TH hour to select logic
            const thTime = new Date(ts + 7 * 3600 * 1000);
            const thHour = thTime.getUTCHours(); // TH hour
        ...
        console.log(`Successfully generated ${numSales} sales for Sept 2.`);
        process.exit(0);
