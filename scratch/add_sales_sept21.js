...
async function addSalesSept21() {
    try {
        console.log('Starting to add sales for Sept 21...');
        ...
        const numSales = Math.floor(Math.random() * 21) + 30; // 30-50 sales
        console.log(`Generating ${numSales} sales...`);
        
        // Generate timestamps for Sept 21, 06:00 - 23:00 (TH Time)
        // UTC: 2026-09-20 23:00:00 to 2026-09-21 15:59:59
        const timestamps = [];
        const minTime = new Date('2026-09-20T23:00:00Z').getTime(); 
        const maxTime = new Date('2026-09-21T15:59:59Z').getTime();
        ...
        // Fetch current inventory and filter out products restocked on Sept 21 (TH Time)
        const [inventoryRows] = await db.query(`
            ...
            AND p.product_id NOT IN (
                SELECT product_id 
                FROM stock_in 
                WHERE DATE(DATE_ADD(created_at, INTERVAL 7 HOUR)) >= '2026-09-21'
            )
        `);
        ...
        console.log(`Successfully generated ${numSales} sales for Sept 21 with strict inventory and logical checks.`);
        process.exit(0);
