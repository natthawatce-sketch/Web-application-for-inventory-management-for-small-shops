...
async function addSalesSept3() {
    try {
        console.log('Starting to add sales for Sept 3...');
        ...
        // Generate timestamps for Sept 3, 06:00 - 23:00 (TH Time)
        // UTC: 2026-09-02 23:00:00 to 2026-09-03 15:59:59
        const timestamps = [];
        const minTime = new Date('2026-09-02T23:00:00Z').getTime(); 
        const maxTime = new Date('2026-09-03T15:59:59Z').getTime();
        ...
        // Let's identify alcohol or water specifically to give them a boost inside the Drinks category (catId 1)
        products.forEach(p => {
            if (!productsByCat[p.category_id]) productsByCat[p.category_id] = [];
            
            // Artificial weighting: if it's alcohol, water, or popular snacks, we add it multiple times to the array to increase its chance of being picked.
        ...
        function getWeightsForHour(hour) {
            if (hour >= 6 && hour < 10) {
                return [
                    { catId: 1, weight: 55 }, // Drinks heavily boosted
                    { catId: 2, weight: 40 }, // Food
                    { catId: 4, weight: 3 },  // Utilities
                    { catId: 3, weight: 1 },  // Condiments
                    { catId: 5, weight: 1 }   // Medicine
                ];
            } else if (hour >= 10 && hour < 16) {
                return [
                    { catId: 1, weight: 50 }, // Drinks
                    { catId: 2, weight: 45 }, // Food
                    { catId: 4, weight: 3 }, 
                    { catId: 3, weight: 1 },  
                    { catId: 5, weight: 1 }   
                ];
                ...
                return [
                    { catId: 1, weight: 40 }, // Drinks (Alcohol starts picking up)
                    { catId: 3, weight: 30 }, // Condiments
                    { catId: 2, weight: 20 }, // Food
                    { catId: 4, weight: 8 }, 
                    { catId: 5, weight: 2 }   
                ];
            } else {
                // 20:00 - 23:00
                return [
                    { catId: 1, weight: 50 }, // Drinks (Alcohol peak)
                    { catId: 2, weight: 40 }, // Food
                    { catId: 4, weight: 5 }, 
                    { catId: 5, weight: 4 },  
                    { catId: 3, weight: 1 }   
        ...
        for (const ts of timestamps) {
            ...
            for (let j = 0; j < numItems; j++) {
                ...
                // Quantity > 1 randomly (e.g. 1 to 3 of the same item)
        ...
        console.log(`Successfully generated ${numSales} sales for Sept 3.`);
        process.exit(0);
