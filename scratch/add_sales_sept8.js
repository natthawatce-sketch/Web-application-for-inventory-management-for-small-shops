...
async function addSalesSept8() {
    try {
        console.log('Starting to add sales for Sept 8...');
        ...
        // Generate timestamps for Sept 8, 06:00 - 23:00 (TH Time)
        // UTC: 2026-09-07 23:00:00 to 2026-09-08 15:59:59
        const timestamps = [];
        const minTime = new Date('2026-09-07T23:00:00Z').getTime(); 
        const maxTime = new Date('2026-09-08T15:59:59Z').getTime();
        ...
        // Fetch current inventory and filter out products restocked on Sept 8 (TH Time)
        const [inventoryRows] = await db.query(`
            ...
            AND p.product_id NOT IN (
                SELECT product_id 
                FROM stock_in 
                WHERE DATE(DATE_ADD(created_at, INTERVAL 7 HOUR)) >= '2026-09-08'
            )
        `);
        ...
        console.log(`Successfully generated ${numSales} sales for Sept 8 with strict inventory checks.`);
        process.exit(0);
