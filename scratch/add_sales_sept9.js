...
async function addSalesSept9() {
    try {
        console.log('Starting to add sales for Sept 9...');
        ...
        // Generate timestamps for Sept 9, 06:00 - 23:00 (TH Time)
        // UTC: 2026-09-08 23:00:00 to 2026-09-09 15:59:59
        const timestamps = [];
        const minTime = new Date('2026-09-08T23:00:00Z').getTime(); 
        const maxTime = new Date('2026-09-09T15:59:59Z').getTime();
        ...
        // Fetch current inventory and filter out products restocked on Sept 9 (TH Time)
        const [inventoryRows] = await db.query(`
            ...
            AND p.product_id NOT IN (
                SELECT product_id 
                FROM stock_in 
                WHERE DATE(DATE_ADD(created_at, INTERVAL 7 HOUR)) >= '2026-09-09'
            )
        `);
        ...
        console.log(`Successfully generated ${numSales} sales for Sept 9 with strict inventory checks.`);
        process.exit(0);
